...
class _FlutterPainterExampleState extends State<PainterHome> {
...
// Obtains an image from network and creates a [ui.Image] object
    ...
    final ui.Image myImage = await FileImage(file).image;
  ...
  void colorBackground(){
    //Sets the background to the color White
  ...
  static PhosphorIconData Function([PhosphorIconsStyle style]) getShapeIcon(ShapeFactory? shapeFactory) {
  ...
  Future<void> renderAndDisplayImage() async {
    ...
    if(!(await Permission.storage.status.isGranted))
      await Permission.storage.request();
    final result = await ImageGallerySaver.saveImage(
        Uint8List.fromList(imageFuture! as List<int>),
        quality: 60,
        name: 'canvas_image'
    );
